...
{		
	...
	public:
		...
		TEST_METHOD(test_Table_is_Union_Compatible_True)
		{
			...
			Assert::IsTrue(clients.is_Union_Compatible(accounts));
		}
		...
		TEST_METHOD(test_Table_Addition)
		{
			...
			Table addition = clients + accounts;

			Assert::IsTrue(addition.get_Tuples().size() == 5);
			Assert::IsTrue(addition.get_Tuples()[2] == accounts.get_Tuples()[0]);
		}
		...
		Table get_Accounts_Table()
		{
			...
			Attribute *client_id3 = new Attribute(321, true, "Client Id");
